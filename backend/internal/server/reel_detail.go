...
func (s *Server) mediaClips(w http.ResponseWriter,r *http.Request) {
    ...
    rows,err:=s.db.Query(r.Context(), `
        SELECT rl.id::text,rl.caption,rl.playback_url,rl.cover_url,rl.duration_ms,
         ...
         ORDER BY
           ...
           rl.published_at DESC NULLS LAST,
           rl.created_at DESC
         LIMIT 16
    `,mediaID)
    if err!=nil {
    ...
    for rows.Next() {
        ...
        if err:=rows.Scan(
            ...
            &likes,&comments,&saves,&shares,&views,&spoiler,
            &authorID,&displayName,&username,&avatar,&verified,
            &catalogID,&tmdbID,&kind,&title,&originalTitle,&poster,&backdrop,&year,&rating,
        ); err!=nil {
            continue
        }
